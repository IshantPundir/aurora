...
use crate::{
    ...
    render::*,
    shell::WindowElement,
    state::{post_repaint, take_presentation_feedback, AuroraState, Backend},
};
...
pub fn run_udev() {
     ...
     */
    let primary_gpu = if let Ok(var) = std::env::var("Aurora_DRM_DEVICE") {
        ...
        .insert_source(notifier, move |event, &mut (), data| match event {
            ...
            SessionEvent::ActivateSession => {
                    ...
                    .map(|(handle, backend)| (*handle, backend))
                {
                    ...
                    if let Some(lease_global) = backend.leasing_global.as_mut() {
                        lease_global.resume::<AuroraState<UdevData>>();
                    }
    ...
    let global = dmabuf_state
        .create_global_with_default_feedback::<AuroraState<UdevData>>(&display_handle, &default_feedback);
        ...
        .and_then(|x| x.ok())
    {
        if let Some(backend) = state.backend_data.backends.get(&primary_node) {
            ...
            if supports_syncobj_eventfd(&import_device) {
                let syncobj_state =
                    DrmSyncobjState::new::<AuroraState<UdevData>>(&display_handle, import_device);
...
impl DrmLeaseHandler for AuroraState<UdevData> {
...
delegate_drm_lease!(AuroraState<UdevData>);

impl DrmSyncobjHandler for AuroraState<UdevData> {
    fn drm_syncobj_state(&mut self) -> &mut DrmSyncobjState {
        self.backend_data.syncobj_state.as_mut().unwrap()
    }
}
smithay::delegate_drm_syncobj!(AuroraState<UdevData>);
...
impl Drop for SurfaceData {
    fn drop(&mut self) {
        if let Some(global) = self.global.take() {
            self.dh.remove_global::<AuroraState<UdevData>>(global);
        }
    }
}
...
impl AuroraState<UdevData> {
    fn device_added(&mut self, node: DrmNode, path: &Path) -> Result<(), DeviceAddError> {
        ...
        let registration_token = self
            ...
            .insert_source(
                notifier,
                move |event, metadata, data: &mut AuroraState<_>| match event {
        ...
        self.backend_data.backends.insert(
            ...
            BackendData {
                ...
                leasing_global: DrmLeaseState::new::<AuroraState<UdevData>>(&self.display_handle, &node)
                    .map_err(|err| {
    ...
    fn connector_connected(&mut self, node: DrmNode, connector: connector::Info, crtc: crtc::Handle) {
        ...
        if non_desktop {
            ...
            if let Some(lease_state) = device.leasing_global.as_mut() {
                lease_state.add_connector::<AuroraState<UdevData>>(
                    connector.handle(),
                    output_name,
                    format!("{} {}", make, model),
                );
            ...
            let global = output.create_global::<AuroraState<UdevData>>(&self.display_handle);
            ...
            let color_formats = if std::env::var("Aurora_DISABLE_10BIT").is_ok() {
                SUPPORTED_FORMATS_8BIT_ONLY
            } else {
                SUPPORTED_FORMATS
            };

            let compositor = if std::env::var("Aurora_DISABLE_DRM_COMPOSITOR").is_ok() {
                let gbm_surface =
                ...
                let disable_direct_scanout = std::env::var("Aurora_DISABLE_DIRECT_SCANOUT").is_ok();
    ...
    fn device_removed(&mut self, node: DrmNode) {
        ...
        if let Some(mut backend_data) = self.backend_data.backends.remove(&node) {
            if let Some(mut leasing_global) = backend_data.leasing_global.take() {
                leasing_global.disable_global::<AuroraState<UdevData>>();
            }
    ...
    fn schedule_initial_render(
        ...
        node: DrmNode,
        crtc: crtc::Handle,
        evt_handle: LoopHandle<'static, AuroraState<UdevData>>,
    ) {
        let device = if let Some(device) = self.backend_data.backends.get_mut(&node) {
            device
        } else {
