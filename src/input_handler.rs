...
use crate::{focus::PointerFocusTarget, shell::FullscreenSurface, AuroraState};
...
impl<BackendData: Backend> AuroraState<BackendData> {
    pub fn process_input_event_windowed<B: InputBackend>(&mut self, event: InputEvent<B>, output_name: &str) {
        match event {
            ...
            _ => (), // other events are not handled in Aurora (yet)
        }
    }
...
impl AuroraState<UdevData> {
    pub fn process_input_event<B: InputBackend>(&mut self, dh: &DisplayHandle, event: InputEvent<B>) {
        match event {
            ...
            _ => {
                // other events are not handled in Aurora (yet)
            }
        }
    }
...
fn process_keyboard_shortcut(modifiers: ModifiersState, keysym: Keysym) -> Option<KeyAction> {
    ...
    } else if modifiers.shift && keysym == Keysym::Return {
        // run terminal
