...
impl DmabufHandler for AuroraState<X11Data> {
    ...
    fn dmabuf_imported(&mut self, _global: &DmabufGlobal, dmabuf: Dmabuf, notifier: ImportNotifier) {
        if self.backend_data.renderer.import_dmabuf(&dmabuf, None).is_ok() {
            let _ = notifier.successful::<AuroraState<X11Data>>();
        } else {
            notifier.failed();
        }
    }
}
delegate_dmabuf!(AuroraState<X11Data>);
...
pub fn run_x11() {
    ...
    let skip_vulkan = std::env::var("Aurora_NO_VULKAN")
        .map(|x| {
    ...
    let dmabuf_global = dmabuf_state.create_global_with_default_feedback::<AuroraState<X11Data>>(
        &display.handle(),
    ...
    let _global = output.create_global::<AuroraState<X11Data>>(&display.handle());
