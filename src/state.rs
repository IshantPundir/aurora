...
impl<BackendData: Backend> SeatHandler for AuroraState<BackendData> {
    ...
    fn seat_state(&mut self) -> &mut SeatState<AuroraState<BackendData>> {
        &mut self.seat_state
    }
...
impl<BackendData: Backend> FractionalScaleHandler for AuroraState<BackendData> {
    fn new_fractional_scale(
        ...
        // of the space (which in case of Aurora will also be the output a toplevel will
...
#[cfg(feature = "xwayland")]
impl<BackendData: Backend + 'static> XWaylandKeyboardGrabHandler for AuroraState<BackendData> {
...
#[cfg(feature = "xwayland")]
delegate_xwayland_keyboard_grab!(@<BackendData: Backend + 'static> AuroraState<BackendData>);
...
smithay::delegate_xdg_foreign!(@<BackendData: Backend + 'static> AuroraState<BackendData>);

smithay::delegate_single_pixel_buffer!(@<BackendData: Backend + 'static> AuroraState<BackendData>);
        ...
        let ret = self
            ...
            .insert_source(xwayland, move |event, _, data| match event {
                XWaylandEvent::Ready {
                    ...
                    let xwayland_scale = std::env::var("Aurora_XWAYLAND_SCALE")
                        .ok()
